...
set -g OSH $HOME/.local/share/bash/oh-my-bash
...
set -g RUNTIME_DIRECTORY /run/user/$UID
...
set -g DOCKER_SOCK /run/docker.sock
set -g DOCKER_SOCK /run/docker/docker.sock
set -g DOCKER_SOCK {$XDG_RUNTIME_DIR}/docker.sock
...
set -g CASJAYSDEVDIR $HOME/.local/share/CasjaysDev/scripts
set -g TMP_BIN_PATH $CASJAYSDEVDIR:$TMP_BIN_PATH
...
set -g SET_USR_PATH "$(fixFishPath user "$TMP_BIN_PATH" "$USRBINDIR" "$FNM_MULTISHELL_PATH")"
set -g PATH "$FNM_MULTISHELL_PATH/bin:$SET_USR_PATH:$USRBINDIR:$SYSBINDIR:."
set -e TMP_BIN_PATH
